...
const UNAS_BASE = (process.env.UNAS_API_URL || 'https://api.unas.eu/shop').replace(/\/+$/, '');
const UNAS_TIMEOUT_MS = Number(process.env.UNAS_TIMEOUT_MS ?? 120000);
const UNAS_DOWNLOAD_TIMEOUT_MS = Number(process.env.UNAS_DOWNLOAD_TIMEOUT_MS ?? 300000);
...
async function unasLogin(apiKey, webshopInfo = true) {
  ...
  const xmlRequest = `<Params><ApiKey>${apiKey}</ApiKey><WebshopInfo>${webshopInfo ? 'true' : 'false'}</WebshopInfo></Params>`;
...
async function requestProductDbLink(token) {
...
<Params>
  ...
  if (resp.status !== 200) {
    throw new AppError(`getProductDB generálás sikertelen HTTP ${resp.status} — ${String(resp.data).slice(0,400)}`, 'UNAS_PRODUCTDB_GEN');
  }

  const link = extractLinkFromXmlOrText(resp.data);
  if (!link) throw new AppError('getProductDB: hiányzik a letöltési link (Url/Link).', 'UNAS_PRODUCTDB_LINK');
  return link;
}

async function fetchProductDbHeaders({ apiKey }) {
  ...
  const link = await requestProductDbLink(token);
  ...
  if (dl.status !== 200) {
    throw new AppError(`getProductDB letöltés sikertelen HTTP ${dl.status}`, 'UNAS_PRODUCTDB_DL');
  }
    ...
    for (let i = 0; i < line.length; i++) {
      ...
      if (ch === '"') {
        if (inQuotes && line[i+1] === '"') {
          cur += '"'; i++; // duplázott idéző → egy idéző karakter
        } else {
          inQuotes = !inQuotes;
        }
