<template>
  <el-container>
    ...
    <el-main>
      <el-button type="primary" class="new-process-btn" @click="openForm()">Új szinkron létrehozása</el-button>
      <ProcessTable
...
export default {
  ...
  setup() {
    ...
    const openForm = (processId = null) => {
      if (processId) {
      const found = Array.isArray(processes.value) ? processes.value.find(p => p.processId === processId) : null
      editedProcess.value = found ? { ...found } : {}
      } else {
      editedProcess.value = {
        processId: '', shopId: '', displayName: '', feedUrl: '',
        frequency: '1h', currency: 'EUR',
        targetCurrency: 'HUF', pricingFormula: '', vat: 27,
        priceMargin: 0, discount: 0, rounding: 100, fieldMapping: {}
      }
      }
      showForm.value = true
    }
    ...
    return {
      shops, selectedShop, processes, logs,
      filteredProcesses, showForm, editedProcess,
      openForm, saveProcess, deleteProcess, loadLogs
    }
  }
}
</script>
