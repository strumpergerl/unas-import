	...
	<div class="p-4" style="margin-top: 5rem;">
		...
		>
				...
				<template #default="{ row }">
					<div class="p-2">
						...
						<el-table :data="row.items" border size="small" row-key="sku">
							...
							<el-table-column prop="unasKey" label="Unas kulcs" min-width="140" />
								...
								<template #default="{ row: it }">
									<div v-if="hasChanges(it)" class="space-y-1">
										...
										>
											<strong>{{ name }}:</strong>
											<span
												class="line-through text-gray-500 mr-1"
			...
			<el-table-column label="Skip" width="90" align="center">
				<template #default="{ row }"
					><el-tag type="warning">{{
						(row.counts?.skippedNoKey || 0) + (row.counts?.skippedNotFound || 0)
					}}</el-tag></template
				>
			</el-table-column>
